...
def report_precision_se_sp_yi_standard_format(y_predicitions,  groundture, loss, fold, epoch, stepname, model_name):
    ...
    for i in range(class_list1.shape[0]):
        ...
        f1 = 0 if pre+SE[i]==0 else 2*pre*SE[i]/(pre+SE[i])
        metrics.append({
            'fold': fold,
            'epoch': epoch,
            'status': stepname,
