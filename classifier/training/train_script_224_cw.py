#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# train_script_224_cw.py
#
# Class-weighted variant of train_script_224.py.
# ONLY DIFFERENCE: adds optional `class_weights` parameter to
# `run_full_ldl_experiment`. When provided, the KL losses are
# weighted per-sample by inverse class frequency.
#
# When class_weights=None, behavior is IDENTICAL to the original.
#
...
def run_full_ldl_experiment(
    ...
    *,
    ...
    """
    LDL trainer with optional per-sample class weighting.

    When class_weights is None, behavior is identical to train_script_224.py.
    When provided (list of 4 floats, one per class), the KL losses are
    weighted per-sample by the weight of each sample's ground-truth class.

    Weights should be inverse-frequency balanced:
        ...
        for ep in range(int(max_epochs)):
            ...
            try:
                ...
                for bx, by, bl, _ in L_tr:
                    ...
                    # ─── Loss computation (with or without class weighting) ───
                    if cw_tensor is not None:
                        # Per-sample weighted KL losses.
                        # by = ground-truth class labels (0-3) from DataLoader.
                        by_gpu = by.cuda(non_blocking=True)
                        w = cw_tensor[by_gpu]  # (B,) per-sample weight

                        # KL with reduction='none' -> (B, C), sum over C -> (B,)
                        # Then multiply by per-sample weight and average over batch.
