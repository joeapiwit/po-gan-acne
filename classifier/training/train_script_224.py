...
# -*- coding: utf-8 -*-
# train_script_224.py
# LDL trainer with safe fixed-label metrics.
# Resize 256 -> RandomCrop 224 for training.
# Resize 224 for evaluation.
...
# Fixed class set for Acne04 / AcneSCU severity classification.
# This prevents macro-F1 from changing denominator when a class is missing.
CLASS_IDS = (0, 1, 2, 3)
...
def _safe_div(num: float, den: float) -> float:
    """
    Safe metric division.
    If denominator is zero, return 0.0.
    Never return NaN.
    Never return 1.0 by default for undefined precision.
    """
    num = float(num)
    den = float(den)
...
def compute_classification_metrics_sane(
    ...
    """
    Safe fixed-label classification metrics.

    This avoids:
      - NaN precision, recall, specificity, or F1
      - inflated precision when no positives are predicted
      - variable macro denominator when a class is absent
      - sklearn zero_division=1 pathology

    Macro values are always averaged over the fixed class_ids.
    """
...
def _sanitize_metric_row(row: dict) -> dict:
    """
    Replace NaN or inf in numeric metric fields.
    Keeps strings unchanged.
    """
    out = dict(row)
...
def evaluation_metrics(model, loader, tag, loss_func, fold, epoch, eval_head: str):
    """
    Evaluate one split and return:
      - metrics_list: rows written to eval_ldl.csv
      - records: per-image predictions
      - (y_true, y_pred): arrays for external use

    Important:
      The final macro metrics are computed by compute_classification_metrics_sane.
      Legacy report columns are overwritten with sane macro values for safety.
    """
    with torch.no_grad():
        v_loss = 0.0
        ...
        # Legacy report calls are retained for compatibility, but their macro columns
        # are not trusted for optimizer fitness.
        try:
        ...
        for row in metrics_list:
            ...
            # Sane metric columns used by optimizer and paper.
            ...
            # These are overwritten so accidental legacy readers do not use unsafe values.
            row["precision"] = float(sane["precision_macro_sane"])
            row["recall"] = float(sane["recall_macro_sane"])
            row["specificity"] = float(sane["specificity_macro_sane"])
            row["f1"] = float(sane["f1_macro_sane"])
...
def calculate_val_macro(df: pd.DataFrame, epoch: int, fold: str):
    """
    Calculate validation macro metrics from current epoch dataframe.
    This reads sane columns only where available.
    """
    d = df[
        (df["epoch"] == int(epoch))
        & (df["status"] == "val")
    ...
    # repo_f1 is kept only for old comparison. It should equal sane f1 now.
    ...
    *,
    ...
    for fold in folds_iter:
        ...
        for ep in range(int(max_epochs)):
            ...
            summary = (
                f"LDL fold {fold} | train {ep:3d} | loss {_safe_float(m_loss.avg, 0.0):.3f} | "
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Override: directory where images actually live",
    )
    ap.add_argument(
        "--folds",
        type=str,
        default=None,
        help="Comma-separated folds to run, e.g. '4' or '0,2,4'. Omit to run all.",
    )
